...
def entry_integrated_cleaning(
    ...
    if (not isinstance(entry, Entry)) or (len(entry.chains) == 0):
        return (
            pd.DataFrame(),
...
def integrated_chainwise_filter(chain: pd.DataFrame) -> Optional[pd.DataFrame]:
...
def disorder_check(chain: pd.DataFrame) -> Optional[pd.DataFrame]:
    ...
    res: pd.DataFrame = pd.concat([dup, partial], ignore_index=True).drop_duplicates()
    return res


def clash_check(chain: pd.DataFrame, lower_bound: float = 1) -> Optional[pd.DataFrame]:
...
def residue_continuity_check(chain: pd.DataFrame) -> Optional[pd.DataFrame]:
...
def residue_completeness_check(chain: pd.DataFrame) -> Optional[pd.DataFrame]:
...
def standard_residue_check(
    chain: pd.DataFrame, label_length: int = 3
) -> Optional[pd.DataFrame]:
...
def field_check(entry_id: str) -> Entry | dict[str, str]:
