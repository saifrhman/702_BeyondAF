...
# 37. Merge Worker Must Not Infer the Repository from Slurm Spool Paths

The first manual production merge attempt exposed the same Slurm execution-path
hazard previously identified for quality workers.

Initial manual merge job:
...
`python: can't open file '/var/spool/scripts/pdbclean/merge_quality_outputs.py'`

The merge batch script had inferred the repository location from
`BASH_SOURCE[0]`. Under real Slurm execution, the submitted batch script is
copied into a spool directory, so the script path no longer points into the
Git checkout.

The merge interface was corrected so that
`merge_quality_outputs.sbatch` receives the repository root explicitly as a
third positional argument:

`CONFIG_PATH MANIFEST_PATH REPOSITORY_ROOT`

The batch worker now:

- resolves the explicitly supplied repository root;
- validates that the repository exists;
- validates that the merge runner exists below that repository;
- executes the merge runner from the explicit checkout;
- does not use `BASH_SOURCE[0]` to infer the repository.

The submission wrapper also passes the repository root explicitly to the merge
job.

A regression test executes a copied merge batch script from a synthetic
`/var/spool/slurmd` location to reproduce the real Slurm condition.

Implementation checkpoint:

`13c809a Pass repository root to merge worker`

Validation after the fix:

- targeted submission tests: 9 passed
- full repository test suite: 298 passed

This is an orchestration correction only and does not alter quality-cleaning
semantics or scientific outputs.

---

# 38. Quality Producer and Merge Validator Require Separate Git Provenance

The production quality shards were generated by Git commit:

`5412c2ffcf70f9747d4482aef9009628cccdbef3`

Subsequent fixes were required before the merge could be executed safely.
Therefore the Git checkout performing the final merge was necessarily newer
than the Git checkout that produced the quality-task shards.

The original merge validation treated one Git commit as both:

- the expected producer commit recorded in every quality shard and task
  summary;
- the commit of the code currently executing the merge.

That model was incorrect. A scientifically valid merge must preserve both
pieces of provenance independently.

The merge provenance model was therefore split into:

- `quality_pipeline_git_commit`: the commit that produced the quality-task
  outputs;
- `merge_pipeline_git_commit`: the clean Git commit that executed and validated
  the merge.

For backward compatibility, `pipeline_git_commit` remains present and aliases
the quality producer commit.

The merge now derives the quality producer commit from the complete set of
task summaries and requires:

- exactly one unique producer commit;
- a valid 40-character hexadecimal Git SHA;
- every task summary and Parquet shard to match that producer commit.

The merge executor's clean Git SHA is recorded separately as merge provenance.

The global quality summary and `_SUCCESS` marker therefore record all three
fields:
...
The quality global-summary and success-marker schema versions were increased
to version `1.1` to represent this provenance contract explicitly.

Implementation checkpoint:

`e707c30 Separate quality and merge provenance`

Validation after the provenance split:

- full repository test suite: 300 passed

The final production merge subsequently completed successfully as Slurm job
`10194689` with exit code `0:0`.
...
- quality producer:
  `5412c2ffcf70f9747d4482aef9009628cccdbef3`
- merge validator:
  `e707c3031a491935ca904af4e80dd85e5856dc1e`

This distinction allows valid immutable quality outputs to be merged by a
later bug-fixed validator without rewriting or misrepresenting the commit that
actually produced those scientific outputs.
