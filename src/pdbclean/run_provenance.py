...
class RunProvenance:
    ...
    def create(
        ...
        *,
        ...
        # The executable projection is written at freeze time, beside the
        # canonical document it is derived from. Workers read this file; they
        # never re-resolve defaults, a profile or a browser form, so a later
        # edit to any of those cannot change what this run means.
        from pdbclean.stage_config import write_stage_config

        stage_config_paths = write_stage_config(resolved, run_dir)
        ...
        record: dict[str, Any] = {
            ...
            "stage_config_path": stage_config_paths["stage_config_path"],
            "stage_config_sha256": stage_config_paths["stage_config_sha256"],
...
#: A run identifier is generated by :func:`new_run_id`; nothing else is
#: accepted, so a run id can never reach the filesystem or an argv as a path.
RUN_ID_PATTERN = re.compile(r"run-\d{8}T\d{6}Z-[0-9a-f]{8}")


class FrozenRunError(RunProvenanceError):
    """Raised when a frozen run cannot be loaded or fails verification."""


@dataclass(frozen=True)
class FrozenRun:
    """A read-only handle on one frozen run.

    Everything that submits or executes scientific work goes through this
    object, and this object reads only the run's own directory.  That is the
    whole guarantee: after a run is frozen, no later edit to
    ``pdbclean.defaults``, to a profile YAML, or to a browser form can reach
    the values a worker computes with.
    """
    ...
    @classmethod
    def load(cls, run_dir: str | Path) -> "FrozenRun":
        directory = Path(run_dir)
        record_path = directory / "run.json"

        if not record_path.is_file():
            raise FrozenRunError(f"No run.json in {directory}")

        try:
            record = json.loads(record_path.read_text(encoding="utf-8"))
        except ValueError as exc:
            raise FrozenRunError(
                f"Unreadable run record: {record_path}: {exc}"
            ) from exc
    ...
    @classmethod
    def locate(cls, run_root: str | Path, run_id: str) -> "FrozenRun":
        if not RUN_ID_PATTERN.fullmatch(run_id):
            raise FrozenRunError(f"Not a valid run identifier: {run_id!r}")

        return cls.load(Path(run_root) / run_id)

    # -- frozen identity ------------------------------------------------

    @property
    def resolved(self) -> ResolvedRunConfig:
        """The canonical configuration, loaded -- never re-resolved."""
    ...
    def git_commit(self) -> str | None:
        return (self.record.get("git") or {}).get("commit")

    @property
    def git_working_tree_dirty(self) -> bool | None:
        return (self.record.get("git") or {}).get("working_tree_dirty")

    @property
    def resolved_config_sha256(self) -> str | None:
        return self.record.get("resolved_config_sha256")

    @property
    def scientific_config_sha256(self) -> str | None:
        return self.record.get("scientific_config_sha256")

    @property
    def snapshot_id(self) -> str | None:
        return (self.record.get("snapshot") or {}).get("snapshot_id")

    @property
    def stage_config_path(self) -> Path:
        from pdbclean.stage_config import stage_config_path

        recorded = self.record.get("stage_config_path")

        return Path(recorded) if recorded else stage_config_path(self.run_dir)

    @property
    def is_legacy(self) -> bool:
        """True for runs frozen before the executable projection existed.

        Such runs are readable historical records.  They are never rewritten
        and never back-filled -- they simply cannot be executed, because the
        configuration their workers would have consumed was not stored.
        """

        return not self.stage_config_path.is_file()

    # -- verification ---------------------------------------------------

    def verify(self) -> dict[str, Any]:
        """Prove the frozen configuration is intact.

        Recomputes both canonical hashes from the stored document and
        re-projects the executable configuration, comparing every result with
        what the run recorded.  Raises before any caller can execute.
        """

        from pdbclean.runconfig import config_sha256, scientific_sha256
        from pdbclean.stage_config import StageConfigError, verify_stage_config

        if self.is_legacy:
            raise FrozenRunError(
                f"Run {self.run_id} is a legacy record: it has no "
                f"{self.stage_config_path.name}, so the configuration its "
                "workers would consume was never stored. It can be inspected "
                "but not executed."
            )

        resolved = self.resolved

        recomputed_resolved = config_sha256(resolved.data)
        recomputed_scientific = scientific_sha256(resolved.data)

        if recomputed_resolved != self.resolved_config_sha256:
            raise FrozenRunError(
                "Resolved configuration hash mismatch for run "
                f"{self.run_id}: recorded {self.resolved_config_sha256}, "
                f"recomputed {recomputed_resolved}. The frozen configuration "
                "has been modified; refusing to execute."
            )

        if recomputed_scientific != self.scientific_config_sha256:
            raise FrozenRunError(
                "Scientific configuration hash mismatch for run "
                f"{self.run_id}: recorded {self.scientific_config_sha256}, "
                f"recomputed {recomputed_scientific}. Refusing to execute."
            )

        try:
            stage_config = verify_stage_config(self.run_dir, resolved)
        except StageConfigError as exc:
            raise FrozenRunError(str(exc)) from exc

        recorded_stage_sha = self.record.get("stage_config_sha256")

        if (
            recorded_stage_sha
            and recorded_stage_sha != stage_config["stage_config_sha256"]
        ):
            raise FrozenRunError(
                "Stage configuration hash mismatch for run "
                f"{self.run_id}: recorded {recorded_stage_sha}, observed "
                f"{stage_config['stage_config_sha256']}. Refusing to execute."
            )

        return {
            "run_id": self.run_id,
            "resolved_config_sha256": recomputed_resolved,
            "scientific_config_sha256": recomputed_scientific,
            **stage_config,
        }
