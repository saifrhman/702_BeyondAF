from utils.profiling_callbacks import DataLoadComputeTimer, EpochJSONLLogger, EpochSummaryCSVLogger
...
def main(args):
    ...
    if (args.log_lr):
        ...
        # --- NEW: time data loading vs compute ---
    callbacks.append(DataLoadComputeTimer(log_every_n_steps=50))

    # --- NEW: write per-epoch metrics to append-only file (not overwritten) ---
    callbacks.append(EpochJSONLLogger(out_dir=args.output_dir, filename="epoch_metrics.jsonl"))
    
    callbacks.append(EpochSummaryCSVLogger(args.output_dir, "epoch_summary.csv"))  # ✅新增

    loggers = []

    # ✅ always log locally so metrics exist even without wandb
    loggers.append(CSVLogger(save_dir=args.output_dir, name="csv_logs"))
    ...
    # ✅ 强制用 GPU（Lightning 2.x 推荐写法）
    if args.gpus is not None and args.gpus > 0:
        trainer_args.update({
            "accelerator": "gpu",
            "devices": args.gpus,   # 你单卡就是 1，多卡就填 4 等
        })
    else:
        trainer_args.update({
            "accelerator": "cpu",
    ...
    trainer_args.update({
        ...
        'callbacks': callbacks,
        'logger': loggers,
        "enable_progress_bar": False,   #  stop tqdm overwrite in logs
    })
